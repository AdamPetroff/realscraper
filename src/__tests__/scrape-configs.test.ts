...
const scrapes = expandSharedScrapes(DEFAULT_STORED_SHARED_SCRAPE_CONFIGS);

describe("shared scrape expansion", () => {
  it("builds the IDNES land scrape from typed config", () => {
    const scrape = getScrapeById(scrapes, "idnes-land-2m-czk-olomouc-10-km");
  ...
  it("builds the IDNES house scrape from typed config", () => {
    const scrape = getScrapeById(scrapes, "idnes-house-5-10m-czk-olomouc-10-km");
  ...
  it("builds the OkDrazby apartment scrape from typed config", () => {
    const scrape = getScrapeById(scrapes, "okdrazby-3-6m-czk-brno");
